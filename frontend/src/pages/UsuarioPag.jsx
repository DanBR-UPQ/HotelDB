...
// Usuario: SOLO LECTURA (GET / GET by ID)
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {list.length ? list.map((item, idx) => {
            const id = item.id ?? item.ID ?? item.Id ?? item.id_cliente ?? item.id_habitacion ?? item.id_reserva ?? item.id_pago ?? item.id_servicio ?? idx;
            return (
              <tr key={id}>
                <td style={td}>{id}</td>
